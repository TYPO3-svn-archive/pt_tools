...
require_once t3lib_extMgm::extPath('pt_tools').'res/staticlib/class.tx_pttools_assert.php'; // assert class
    ...
    */
    ...
    public static function returnExtConfArray($extKey, $noExceptionIfNoConfigFound=false) {
        ...
        if (!is_array($baseConfArr)) {
            if ($noExceptionIfNoConfigFound == true) {
                ...
                throw new tx_pttools_exception('Extension configuration in localconf.php for extension "'.$extKey.'" not found!', 2,
    ...
    public static function mergeConfAndFlexform($pObj, $noExceptionIfNoFlexform = false) {
        ...
        if (!is_callable(array($pObj, 'pi_initPIflexForm'))) {
            throw new tx_pttools_exception('"$pObj needs a callable method "pi_initPIflexForm()"');
        }
     ...
     * @throws  tx_pttools_exception   if the SHOW TABLES query fails/returns false
     * @author  Rainer Kuhn <kuhn@punkt.de>
     * @since   2007-01-11
     */
     ...
     * @throws  tx_pttools_exception   if the SHOW TABLE STATUS query fails/returns false
     * @author  Rainer Kuhn <kuhn@punkt.de>
     * @since   2007-03-26
     */
 ...
 ******************************************************************************/
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pt_tools/res/staticlib/class.tx_pttools_div.php']) {
    include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/pt_tools/res/staticlib/class.tx_pttools_div.php']);
}
